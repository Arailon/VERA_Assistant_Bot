...
if cursor.fetchone()["c"] == 0:
    items = [
        ("Тост с ветчиной и сыром", "Хрустящий тост с ветчиной и плавленым сыром", 445, "Еда"),
        ("Veggie Bowl", 350, "Еда"),
        ("Кето-бейгл", "Еда"),
        ("Бриошь с пастрами", "Нежная бриошь с пастрами", 700, "Еда"),
    ]
...
async def main_menu_choice(call: types.CallbackQuery):
    """
    Показываем сначала две опции: посмотреть на сайте или посмотреть здесь.
    По кнопке 'Посмотреть здесь' идёт callback 'menu_here' — который показывает категории.
    """
    await call.answer()  # подтверждаем callback, чтобы убрать spinner
...
async def main_admin_cb(call: types.CallbackQuery):
    try:
        # если не админ — покажем alert и выйдем
        if call.from_user.id not in ADMINS:
            await call.answer("⛔️ Нет доступа", show_alert=True)
            return

        # Важно — немедленно подтвердить callback, чтобы снять spinner
        await call.answer()

        # Попробуем отредактировать сообщение (или отправить новое)
        await call.message.edit_text("⚙️ Админ меню", reply_markup=admin_menu_inline())

    except Exception as e:
        # логирование и уведомление — если что-то пошло не так, спиннер точно снимется
        try:
            await call.answer("Ошибка при открытии админ меню. Проверь логи.", show_alert=True)
        except:
            pass
        import traceback
        traceback.print_exc()
